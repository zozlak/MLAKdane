#' wylicza zadane okienka danych zgodnie z potrzebami projektu ELA
#' @param okienko obiekt okienka utworzony za pomocą funkcji
#'   \code{\link{okienko}}
#' @param zdau dane zwrócone przez funkcję \code{\link{przygotuj_zdau}}
#' @param baza dane zwrócone przez funkcję \code{\link{polacz_zus_zdau}}
...
#' @import dplyr
okienko_ela = function(okienko, zdau, baza, miesieczne, utrataPracy, jednostki, krok = 1000000, info = TRUE) {
  stopifnot(
  ...
  while (TRUE) {
    ...
    okienkoMies = oblicz_okienko(miesieczne, okienko, krokZdau)
    okienkoBaza = oblicz_okienko(baza, okienko, krokZdau)
    ...
    if (okienko[['offsetMax']] == 99) {
      zam0[[length(zam0) + 1]]  =  oblicz_zamieszkanie(okienkoBaza, jednostki, TRUE)
      zam1[[length(zam1) + 1]]  = oblicz_zamieszkanie(okienkoBaza, jednostki, FALSE)
    }
