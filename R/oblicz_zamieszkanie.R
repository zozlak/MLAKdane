...
#'   z danych wygenerowanych wcześniej funkcją \code{\link{polacz_w_baze}})
...
#' @export
#' @import dplyr
oblicz_zamieszkanie = function(dane, jednostki, multidplyr = FALSE){
  stopifnot(
  ...
  dane = dane %>%
    ...
    mutate_(
      jpdzam = ~case_when(
        .$powiat_jedn == floor(.$teryt / 100) ~ 1L,
        .$woj_jedn == floor(.$teryt / 10000) ~ 2L,
        .$teryt > 0 ~ 3L,
        TRUE ~ NA_integer_
      )
    )
