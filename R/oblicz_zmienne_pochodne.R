...
oblicz_zmienne_pochodne = function(dane, multidplyr = TRUE){
  ...
  dane = dane %>%
    mutate_(
      ...
      ezpow_z = ~dplyr::coalesce((ezpow_z_n + ezpow_z_s) / nm_z, NA_real_),
      ezpow_e = ~dplyr::coalesce((ezpow_e_n + ezpow_e_s) / nm_e, NA_real_),
      ...
      ez_z2  = ~dplyr::coalesce(sz_z / len, NA_real_),
      wzg_ez_e     = ~dplyr::coalesce(ez_e / ezpow, NA_real_),
      wzg_ez_e2    = ~dplyr::coalesce(ez_e / ezpow_e, NA_real_),
      wzg_ez_z     = ~dplyr::coalesce(ez_z / ezpow, NA_real_),
      wzg_ez_z2    = ~dplyr::coalesce(ez_z / ezpow_z, NA_real_),
