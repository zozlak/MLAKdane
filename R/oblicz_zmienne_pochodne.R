...
oblicz_zmienne_pochodne = function(dane, multidplyr = TRUE){
  ...
  dane = dane %>%
    mutate_(
      ...
      if_e_n = ~ifelse(nm_x_n > 0, as.integer(nm_e_n > 0L), NA_integer_),
      if_e_s = ~ifelse(nm_x_s > 0, as.integer(nm_e_s > 0L), NA_integer_),
      if_m   = ~as.integer(nm_m > 0L),
      if_p   = ~as.integer(nm_p > 0L),
      if_p_n = ~ifelse(nm_x_n > 0, as.integer(nm_p_n > 0L), NA_integer_),
      if_p_s = ~ifelse(nm_x_s > 0, as.integer(nm_p_s > 0L), NA_integer_),
      if_s   = ~as.integer(nm_s > 0L),
      if_s_n = ~ifelse(nm_x_n > 0, as.integer(nm_s_n > 0L), NA_integer_),
      if_s_s = ~ifelse(nm_x_s > 0, as.integer(nm_s_s > 0L), NA_integer_),
      ...
      pm_bd_n = ~dplyr::coalesce(100L * nm_bd_n / len, NA_real_),
      pm_bd_s = ~dplyr::coalesce(100L * nm_bd_s / len, NA_real_),
      ...
      pm_d_n  = ~dplyr::coalesce(100L * nm_d_n / len, NA_real_),
      pm_d_s  = ~dplyr::coalesce(100L * nm_d_s / len, NA_real_),
      pm_e    = ~dplyr::coalesce(100L * nm_e / len, NA_real_),
      pm_e_n  = ~dplyr::coalesce(100L * nm_e_n / len, NA_real_),
      pm_e_s  = ~dplyr::coalesce(100L * nm_e_s / len, NA_real_),
      pm_i    = ~dplyr::coalesce(100L * nm_i / len, NA_real_),
      pm_i_n  = ~dplyr::coalesce(100L * nm_i_n / len, NA_real_),
      pm_i_s  = ~dplyr::coalesce(100L * nm_i_s / len, NA_real_),
      pm_n    = ~dplyr::coalesce(100L * nm_n / len, NA_real_),
      pm_n_n  = ~dplyr::coalesce(100L * nm_n_n / len, NA_real_),
      pm_n_s  = ~dplyr::coalesce(100L * nm_n_s / len, NA_real_),
      pm_p    = ~dplyr::coalesce(100L * nm_p / len, NA_real_),
      pm_p_n  = ~dplyr::coalesce(100L * nm_p_n / len, NA_real_),
      pm_p_s  = ~dplyr::coalesce(100L * nm_p_s / len, NA_real_),
      pm_s    = ~dplyr::coalesce(100L * nm_s / len, NA_real_),
      pm_s_n  = ~dplyr::coalesce(100L * nm_s_n / len, NA_real_),
      pm_s_s  = ~dplyr::coalesce(100L * nm_s_s / len, NA_real_),
