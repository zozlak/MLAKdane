...
library(dplyr)
dataMin = '2014-01-01'
dataMax = '2015-09-30' # 2015-03-31 dla starych danych
...
for(i in seq_along(okienkaMin)){
  ...
  filtr = colnames(razem)[! colnames(razem) %in% colnames(okienko)]
  razem = razem[, c('id_zdau', filtr)]
  colnames(razem) = paste0(colnames(razem), okienkaSufiksy[i])
  colnames(razem)[1] = 'id_zdau'
...
system.time(oblicz_absolwent(okienko))
