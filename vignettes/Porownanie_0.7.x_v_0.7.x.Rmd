...
library(MLAK)
devtools::load_all(".")
wszystkoS = wczytajDane('../dane/archiwum/2016-09_0.7.1.RData')
miesieczneS = wczytajDane('../dane/archiwum/2016-09_0.7.1_mies.RData')
miesieczneS = miesieczneS %>% rename(ID_ZDAU = ID_ZDAU_M, OKRES = OKRES_M) %>% semi_join(wszystkoS %>% select(ID_ZDAU))
wszystko = wczytajDane('../dane/ZUS_2015-09/2014_dane.RData')
miesieczne = wczytajDane('../dane/ZUS_2015-09/2014_dane_mies.RData')
...
## oblicz_pracodawcy
```{r, echo = TRUE, warning = FALSE, message = FALSE, cache = FALSE}
porownaj(wszystkoS, wszystko, 'ENUP_ENL', 'ENUP_ENL') %>% group_by(d) %>% summarize(n = n()) 
...
# miesięczne
```{r, echo = TRUE, warning = FALSE, message = FALSE, cache = FALSE}
porownaj(miesieczneS, miesieczne, 'WZG_RYZBEZ_M', 'WZG_RYZBEZ_M', c('ID_ZDAU', 'OKRES')) %>% filter(!is.nan(a), abs(d) > 0.0001) # pomijamy nan-y ze względu na błędy wyliczania w wersji 0.6.0
porownaj(miesieczneS, miesieczne, 'WZG_EZ_Z_M', 'WZG_EZ_Z_M', c('ID_ZDAU', 'OKRES')) %>% filter(!is.na(a), abs(d) > 0.0001)  # pomijamy NA, bo w poprzednim zbiorze brakowało paru kodów pocztowych, przez co ZILO wychodziło NA
```
