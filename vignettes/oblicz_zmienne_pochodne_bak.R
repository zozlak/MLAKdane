...
oblicz_zmienne_pochodne = function(dane){
  ...
  dane = dane %>%
    ...
    mutate_(
      ...
      stud        = ~ as.numeric(nmstud > 0),
      studproc    = ~ 100 * nmstud / len,
      zilo        = ~ ezard / zpow,
      ziloa       = ~ ezard / gezd,
      bilod       = ~ pbezd / gbezd,
    ...
    mutate_(
      ...
      studproc  = ~ ifelse(is.infinite(studproc), NA, studproc),
    ...
    mutate_(
      etatdproc = ~ 100 * (1 - mnprud),
      pracdproc = ~ 100 * (1 - mnprd)
    )
