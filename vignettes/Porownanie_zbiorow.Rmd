...
zus %>% filter(id == 196338524) %>% select(okres) %>% distinct() %>% arrange(okres)
```
Wygląda na to, że MS liczy tylko do ostatniej informacji z ZUS-u? (ja liczę do końca rozważanego okresu)  
W sumie zaskakujące, że rozbieżności występują tylko dla 26 osób.
...
```
Wygląda na to, że NMB wg MS to coś pomiędzy naszym NMB (tylko po *bezrob*) a naszym NMB_V2 (*bezrob* i żadno z *etat, netat, samoz, rentemer, student*)
...
porownaj(daneMS, wszystko, 'GBEZD') %>% group_by(abs(d) < 0.1) %>% summarize(n = n())
